...
const MovieList = ({title, movies}) => {
  ...
  return (
    <div className='text-white flex flex-col gap-6'>
        <h1 className='sm:text-3xl text-xl font-bold [text-shadow:1px_1px_2px_#000000]'>{title}</h1>
        <div className='relative'>
            <div ref={container}  className='flex overflow-x-scroll no-scrollbar scroll-smooth'>

                <button onClick={scrollLeft}  className='absolute left-0 bottom-0 bg-black/30 text-white hidden sm:flex items-center justify-center h-full w-20'>
                ...
                <button onClick={scrollRight} className='absolute right-0 top-0 bg-black/30 text-white hidden sm:flex items-center justify-center h-full w-20'>
